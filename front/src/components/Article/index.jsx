...
const Article = ({ name, pathApi, internal_url }) => {
    const { url } = useParams()

    const usequery = useQueryFetcher([name, url], pathApi + url)
    console.log(usequery.data?.current);

    return (usequery.data?.current &&
        <Layer
            ...
            header={{ title: usequery.data.current.titulo, text: `` }}>


            <div class="cont relative z-50 my-20">

                {/* <h2 class="text-left  mt-9 md:mt-16">{usequery.data.current.titulo}</h2> */}


                {/* case article */}
                {usequery.data.current.tag &&
                    <div class="flex gap-6 my-5 ">
                        {usequery.data.current.tag.map((v, k) => (
                            <div key={k} class="flex items-center gap-[10px]">
                                <i>{v.icon}</i>
                                <p class="">{v.nombre}</p>
                            </div>
                        ))}
                ...
                {usequery.data.current.galeria.length > 0 && <Carrusel data={usequery.data.current.galeria} />}

                <article dangerouslySetInnerHTML={{ __html: usequery.data.current.contenido }} />



                <div class="text-center mt-24 pb-6 ">
                ...
                <div class="grid grid-cols-1 gap-6  md:grid-cols-3">
                    {usequery.data.more.length &&

                        usequery.data.more.map((v, k) => (
                            <NavLink className="bg-white flex flex-col  rounded-lg text-black w-auto  "
                                to={internal_url+v.url}>
