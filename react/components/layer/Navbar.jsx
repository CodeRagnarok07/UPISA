...
export default function Navbar({ content }) {
  ...
  const handleClick = (e) => {
    ...
    // bar.current.parentNode.getElementsByTagName("carrusel").classList.toggle("hidden-movil")
    bar.current.parentNode.querySelector(".carrusel").classList.toggle("hidden-movil")
    console.log(bar.current.parentNode);
  }
  ...
  const urls = [
    ...
    {
      url: "/productos/",//"{% url 'productos' %}",
      name: 'productos'
    },
    {
      url: "/parrilleros/",//"{% url 'parrilleros' %}",
      name: 'parrilleros'
    },
    {
      url: "/logo/",//"{% url 'index' %}",
    ...
    {
      url: "/novedades/",//"{% url 'novedades' %}",
      name: 'novedades'
    },
    {
      url: "/recetas/",//"{% url 'recetas' %}",
      name: 'recetas'
    },
    {
      url: "/contacto/",//"{% url 'contacto' %}",
      name: 'contacto'
    },
  ]
  ...
  return (
    <div className={"navbar bg"}>
      ...
      <ul ref={bar}>
        ...
        {urls.map((v, k) => (
          v.name == `logo` ?
            <a key={k} className={"logo"} href={v.url}><img src='/static/img/Logo.png' alt="" /></a> :

            <a key={k} className="hidden-movil" href={langcode + v.url}>{v.name}</a>))}
